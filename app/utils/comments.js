export const POST_DETAILS_MOCK_DATA = [
    {
        id: 1,
        content: 'Nice daug!',
        user: {
            name: 'Ryan Liszewski',
            image: 'https://avatars3.githubusercontent.com/u/5275250?s=460&v=4',
        }
    },
    {
        id: 2,
        content: 'What breed is that dog?',
        user: {
            name: 'Nicholas Szeto',
            image: 'https://avatars1.githubusercontent.com/u/23561635?s=460&v=4',
        }
    },
    {
        id: 3,
        content: 'I do not know exactly, but I love it!',
        user: {
            name: 'Ryan Liszewski',
            image: 'https://avatars3.githubusercontent.com/u/5275250?s=460&v=4',
        }
    },
    {
        id: 4,
        content: 'Yeah, I like it too',
        user: {
            name: 'Nicholas Szeto',
            image: 'https://avatars1.githubusercontent.com/u/23561635?s=460&v=4',
        }
    },
    {
        id: 5,
        content: 'WOW!',
        user: {
            name: 'Jakhongir Khusanov',
            image: 'https://avatars0.githubusercontent.com/u/25942541?s=460&v=4',
        }
    },
    {
        id: 6,
        content: 'I have the same kind of dog',
        user: {
            name: 'Prakash Gurung',
            image: 'https://avatars.githubusercontent.com/makkhay',
        }
    },
    {
        id: 7,
        content: 'Great!',
        user: {
            name: 'Ryan Liszewski',
            image: 'https://avatars3.githubusercontent.com/u/5275250?s=460&v=4',
        }
    },
    {
        id: 8,
        content: 'See my pictures',
        user: {
            name: 'Nicholas Szeto',
            image: 'https://avatars1.githubusercontent.com/u/23561635?s=460&v=4',
        }
    }
]